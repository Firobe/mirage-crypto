...
module type Foreign = sig
  ...
  val scalar_mult_base_c : point -> bytes -> unit
...
module Make_field_element (P : Parameters) (F : Foreign) : Field_element = struct
  ...
  let copy dst src =
    (* XXX(dinosaure): assert (Bytes.length src = Bytes.length dst); *)
...
module Make_point (P : Parameters) (F : Foreign) : Point = struct
  ...
  let check_coordinate buf =
    (* ensure cs < p: *)
    match Eqaf.compare_be_with_len ~len:P.byte_length (Bytes.unsafe_to_string buf) P.p >= 0 with
    | true -> None
...
module Make_scalar (Param : Parameters) (P : Point) : Scalar = struct
  ...
  let is_in_range buf =
    not_zero buf
    && Eqaf.compare_be_with_len ~len:Param.byte_length Param.n (Bytes.unsafe_to_string buf) > 0
  ...
  let bit_at (Scalar s) i =
    ...
    let byte = Char.code (Bytes.get s byte_num) in
...
module Make_dh (Param : Parameters) (P : Point) (S : Scalar) : Dh = struct
  ...
  let secret_of_bytes s =
    match S.of_bytes s with
    | Ok p -> Ok (p, share p)
    | Error _ as e -> e

  let secret_of_cs cs =
    match S.of_bytes (Cstruct.to_bytes cs) with
    | Ok p -> Ok (p, Cstruct.of_bytes (share p))
    | Error _ as e -> e
  ...
  let gen_key ?g () =
    let private_key, share = gen_bytes_key ?g () in
    private_key, Cstruct.of_bytes share

  let key_bytes_exchange secret received =
    match point_of_buf received with
    | Error _ as err -> err
...
module Make_dsa (Param : Parameters) (F : Foreign_n) (P : Point) (S : Scalar) (H : Mirage_crypto.Hash.S) : Dsa = struct
  ...
  let priv_to_cstruct = Cstruct.of_bytes <.> priv_to_bytes
  ...
  let pub_to_bytes = P.to_bytes

  let pub_of_cstruct cs = pub_of_bytes (Cstruct.to_bytes cs)
  let pub_to_cstruct = Cstruct.of_bytes <.> pub_to_bytes
      ...
      if not (S.is_in_range r && S.is_in_range s) then
        ...
        | Ok u1, Ok u2 ->
          ...
          begin match x_of_finite_point_mod_n point with
            ...
            | Some r' -> Bytes.equal r r' (* TODO(dinosaure): [Eqaf]? *)
          end
...
module P224 : Dh_dsa = struct
  module Params = struct
    ...
    let p = Cstruct.to_string (Cstruct.of_hex "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF000000000000000000000001")
    let n = Cstruct.to_string (Cstruct.of_hex "FFFFFFFFFFFFFFFFFFFFFFFFFFFF16A2E0B8F03E13DD29455C5C2A3D")
    let byte_length = 28
  ...
  module Foreign = struct
    ...
    external scalar_mult_base_c : point -> bytes -> unit = "mc_p224_scalar_mult_base" [@@noalloc]
...
module P256 : Dh_dsa = struct
  module Params = struct
    ...
    let p = Cstruct.to_string (Cstruct.of_hex "FFFFFFFF00000001000000000000000000000000FFFFFFFFFFFFFFFFFFFFFFFF")
    let n = Cstruct.to_string (Cstruct.of_hex "FFFFFFFF00000000FFFFFFFFFFFFFFFFBCE6FAADA7179E84F3B9CAC2FC632551")
    let byte_length = 32
    let fe_length = 32
    let first_byte_bits = None
  end
  ...
  module Foreign = struct
    ...
    external scalar_mult_base_c : point -> bytes -> unit = "mc_p256_scalar_mult_base" [@@noalloc]
...
module P384 : Dh_dsa = struct
  module Params = struct
    ...
    let p = Cstruct.to_string (Cstruct.of_hex "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFFFF0000000000000000FFFFFFFF")
    let n = Cstruct.to_string (Cstruct.of_hex "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFC7634D81F4372DDF581A0DB248B0A77AECEC196ACCC52973")
    let byte_length = 48
    let fe_length = 48
    let first_byte_bits = None
  end
  ...
  module Foreign = struct
    ...
    external scalar_mult_base_c : point -> bytes -> unit = "mc_p384_scalar_mult_base" [@@noalloc]
...
module P521 : Dh_dsa = struct
  module Params = struct
    ...
    let p = Cstruct.to_string (Cstruct.of_hex "01FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF")
    let n = Cstruct.to_string (Cstruct.of_hex "01FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFA51868783BF2F966B7FCC0148F709A5D03BB5C9B8899C47AEBB6FB71E91386409")
    let byte_length = 66
  ...
  module Foreign = struct
    ...
    external scalar_mult_base_c : point -> bytes -> unit = "mc_p521_scalar_mult_base" [@@noalloc]
...
module X25519 = struct
  ...
  let scalar_mult in_ base =
    ...
    x25519_scalar_mult_generic out in_ 0 base 0;
    out
  ...
  let gen_key ?g () =
    let secret, public = gen_bytes_key ?g () in
  ...
  let secret_of_cs cs = match secret_of_bytes (Cstruct.to_bytes cs) with
...
module Ed25519 = struct
  ...
  let priv_of_cstruct = priv_of_bytes <.> Cstruct.to_bytes
  ...
  let pub_of_cstruct = pub_of_bytes <.> Cstruct.to_bytes
